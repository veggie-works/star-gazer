...
func _ready() -> void:
	for action in InputMap.get_actions().filter(func(action): return not action.contains("ui_")):
		...
		input_rebinder.joypad_rebound.connect(on_joypad_rebound)
...
## Callback for when an input rebinder's joypad is rebound
func on_joypad_rebound(action_name: String, button: InputEventJoypadButton) -> void:
	for action: String in InputMap.get_actions():
		if action == action_name:
			continue
		var events: Array[InputEvent] = InputMap.action_get_events(action)
